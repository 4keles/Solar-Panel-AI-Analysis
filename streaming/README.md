...
```bash
# Projenin ana dizininde (root klasöründe) olduğunuzdan emin olun
# Webcam ile başlat:
uv run python streaming/main.py --source 0 --model models/latest/best.pt

# Video dosyası + kayıt:
uv run python streaming/main.py --source footage.mp4 --record

# Headless (GUI yok, sadece dry-run testi):
uv run python streaming/main.py --source 0 --dry-run --record
```
