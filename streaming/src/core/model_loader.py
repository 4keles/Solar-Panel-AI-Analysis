...
# Supported model formats and their display labels
_SUPPORTED_EXTENSIONS = {".pt", ".onnx", ".engine"}
...
class ModelLoader:
    ...
    def load(self, model_path: Path) -> YOLO:
        ...
        if ext not in _SUPPORTED_EXTENSIONS:
            logger.warning("unsupported_model_format", ext=ext, path=str(model_path))

        try:
            model_type = {".pt": "PyTorch", ".onnx": "ONNX", ".engine": "TensorRT"}.get(ext, ext)
            logger.info("loading_model", path=str(model_path), format=model_type)

            # Ultralytics YOLO supports .pt, .onnx, and .engine natively
            # For .engine files, TensorRT must be installed on the system
            self._model = YOLO(str(model_path), task="detect")
            self._model_path = model_path

            device = str(self._model.device) if hasattr(self._model, "device") else "unknown"
            logger.info("model_loaded_successfully", format=model_type, device=device)
            return self._model
        except Exception as e:
            logger.error("model_load_failed", path=str(model_path), error=str(e))
            raise ModelLoadError(f"Model yüklenirken bir hata oluştu ({model_path.suffix}): {e}") from e
